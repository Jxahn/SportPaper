...
public abstract class EntityLiving extends Entity {
    ...
    public boolean a(Entity entity, float f, double d0, double d1) { // SportPaper - void -> boolean
        if (this.random.nextDouble() >= this.getAttributeInstance(GenericAttributes.c).getValue()) {
            ...
            double distance = Math.sqrt((this.getX()-entity.getX())*(this.getX()-entity.getX()) + (this.getY()-entity.getY())*(this.getY()-entity.getY()) + (this.getZ()-entity.getZ())*(this.getZ()-entity.getZ()));

            if(distance <= PaperSpigotConfig.knockbackRangeFactor)
            {
                horizontalKnockback = PaperSpigotConfig.knockbackMaxRangeReduction;
            }
